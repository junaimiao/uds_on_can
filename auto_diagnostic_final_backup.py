...
def init():
    ...
    return [r,wb,sht,number,date_today,ch,start_line_number]
        ...
        if str(r.get("print_message_status"),encoding="utf-8") == "start":
            ...
            if frame.id == response_id:
                ...
                if check_result:
                    ...
                    sht.range("%s%s" % ("P",line_number)).value = "huzhicheng"
                    sht.range("%s%s" % ("Q",line_number)).value = date_today
                    sht.range("%s%s" % ("M",line_number)).value = uds.NRC_table[uds.NRC]
                    # 将数据保存到表格
                    wb.save()
                    # 向发送线程设置下一个报文发送开始消息
                    r.set("status","%s%s" % ("start",str(int(line_number)+1)))
...
# 读取excel中的数据并发送can报文
def sendMessage(threadName,ch,number,start_line_number = 11,physical_id = 0x79D,function_id = 0x760):
    # 这个对象需要重新实例化
    wb = xw.Book("./uds_test2.xlsx")
    sht = wb.sheets("1_1_DiagAndCommMgtFuncUnit")
    # 初始化发送包当前时间
    ...
    while i < start_line_number + number:
        if str(r.get("test"),encoding="utf8") == "start":
            if str(r.get("status"),encoding="utf8") == "%s%s" % ("start",str(i)):
                ...
                if sht.range("%s%s" % ("H",str(i))).value == "物理请求":
                ...
                try:
                    ...
                    if sht.range("%s%s" % ("F",str(i))).value == "默认会话":
                        ch.write(uds.session(id=physical_id,sub_function=0x81))
                        time.sleep(0.01)
                    elif sht.range("%s%s" % ("F",str(i))).value == "扩展会话":
                        ch.write(uds.session(id=physical_id,sub_function=0x81))
                        time.sleep(0.01)
                        ch.write(uds.session(id=physical_id,sub_function=0x83))
                        time.sleep(0.01)
                    elif sht.range("%s%s" % ("F",str(i))).value == "编程会话":
                        ch.write(uds.session(id=physical_id,sub_function=0x81))
                        time.sleep(0.1)
                        ch.write(uds.session(id=physical_id,sub_function=0x83))
                        time.sleep(0.01)# 等待10ms
                ...
                if receive_response_time - send_frame_time > 30.0:
                    # 超时判断为无响应
                    sht.range("%s%s" % ("L",str(i-1))).value = "无响应"
                    sht.range("%s%s" % ("M",str(i-1))).value = "-"
                    sht.range("%s%s" % ("O",str(i-1))).value = "OK"
                    sht.range("%s%s" % ("P",str(i-1))).value = "huzhicheng"
                    sht.range("%s%s" % ("Q",str(i-1))).value = date_today
                    sht.range("%s%s" % ("N",str(i-1))).value = "-"
                    # 设置开始发送下一行报文的消息
...
if __name__ == "__main__":
    # 自动诊断脚本初始化(必须初始化的内容)
    [r,wb,sht,number,date_today,ch,start_line_number] = init()
